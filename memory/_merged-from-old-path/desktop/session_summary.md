...
---
## What happened (2026-05-05 â€” Delete Pitch button + Priority Plumbing cleanup)
...
- `lib/automation/github.ts` â€” added `deleteRepo(fullName)`. Treats 404 as already-gone success. Surfaces 403/missing-scope error explicitly so the caller knows to update `GITHUB_TOKEN`.
- `lib/automation/vercel.ts` â€” added `deleteProject(name)`. Same 404-as-success pattern.
- `app/api/admin/prospects/[id]/delete-pitch/route.ts` â€” new POST route. Body `{ mode: 'soft' | 'hard' }`. Soft = wipe artifacts + reset row to `selected`. Hard = wipe artifacts + delete the prospect row entirely. Returns artifacts object with vercel/github/db status so partial failures are visible.
- `app/admin/prospects/[id]/DeletePitchButton.tsx` â€” destructive-looking button (rose color), expands inline to show mode radio + "I understand this is permanent" checkbox. Result panel shows per-step success/failure inline.
- `app/admin/prospects/[id]/page.tsx` â€” wired below ProspectActions, visible only when `pitch_repo || pitch_url` is set.
...
### Priority Plumbing cleanup â€” fully done

| Artifact | State |
|---|---|
| Vercel project | âœ“ deleted via `vercel project remove --scope mako-studi` |
| GitHub repo | âœ“ deleted via `gh repo delete MakoBytes-com/priority-plumbing-pitch --yes` (after Russell ran `gh auth refresh -h github.com -s delete_repo` to expand his local CLI scope) |
| Supabase prospects row | âœ“ hard-deleted via DELETE on `/rest/v1/prospects?id=eq.c50a73a8-...` |
...
**STILL OPEN:** the portal's `GITHUB_TOKEN` env var in Vercel (a separate PAT) only has `repo` scope. Until that's regenerated with `repo + delete_repo` and updated in Vercel env, the Delete Pitch button's GitHub step will 403 with the "needs `delete_repo`" error message. Vercel + DB steps still work, so the button isn't broken â€” just incomplete for cleanup. Russell can update the PAT whenever he's ready (steps documented in this session).

---

## What happened (2026-05-05 â€” name scrub across 5 repos, 2 phases)
...
### Phase 1 â€” user-facing copy (4 commits)
...
Replacement: "Russell" / "Russell's queue" / "Russell sees" â†’ "MakoAI.Studio" / "MakoAI.Studio support team". `placeholder="Russell Sailors"` â†’ `placeholder="Jane Doe"`.

All 4 typecheck-clean before push, all 4 deploys auto-triggered.

### Phase 2 â€” internal references (5 commits, all comment-only + portal stack entry)
...
Sweep was sed-based (`Russell Sailors` â†’ `Mako Admin`, `Russell` â†’ `Mako Admin`, `russell.sailors@gmail.com` â†’ `admin@<domain>`). Verified with grep post-sweep â€” all 5 repos clean of `Russell|russell.sailors` matches.
...
- **`makoai.studio/lib/places.ts:69`** â€” `/\b(russell|sailors)\b/i` regex. This is the privacy-protective FILTER that drops Google Reviews mentioning the founder. Removing it would re-introduce name leaks via live-fetched testimonials. Saved in the rule.
- **`makoai-portal/.env.rls-temp`** â€” untracked local config with `russell.sailors@gmail.com` in the ADMIN_EMAILS list. Looks like a remnant migration file. Skipped because (a) untracked = doesn't ship, (b) actively used as admin auth. Russell can delete or scrub locally.
...
- `TopPaws.com/public/llms.txt:43` â€” public AI-crawler doc names "Russell Sailors as a solo founder project"
- `lagunares-showcase/STARTER_README.md` (PUBLIC GitHub repo) â€” multiple Russell mentions
- `lagunares.com/STARTER_README.md` â€” same content
- `lagunares.com/app/actions/submit-contact.ts:119` + `lagunares-showcase/...:119` â€” code comment with `russell@makologics.com` (work email, different from personal)

**Internal-only:**
- `IDEAS/seo-content-tool/*.md` â€” planning docs with many Russell references
- `IDEAS/mako-growth-plan.md`
- `makobot.com/lib/db.ts` â€” single occurrence, not yet read
...
1. **Phase 3 sweep** â€” extend the name scrub to TopPaws (llms.txt is public), lagunares + lagunares-showcase (public repo), and IDEAS folder. Russell's call.
2. **`.env.rls-temp` cleanup** â€” Russell decides whether to delete the file or trim the email list to drop the personal entry.
