  ...
  return (
    <main className="relative">
      ...
      <section className="relative pt-32 md:pt-40 pb-16 md:pb-20 overflow-hidden">
        ...
        <div className="container-narrow relative">
          ...
          >
            ...
            <span className="mx-2 text-mist-400/50">/</span>
            <span className="text-mist-200">{item.name}</span>
          </nav>

          <Reveal y={18}>
            <span className="section-label">Case study</span>
          </Reveal>
          <Reveal delay={0.15} y={30} blur>
            <h1 className="mt-6 font-display font-medium text-[38px] md:text-[62px] leading-[1.05] tracking-tight max-w-3xl text-mist-100">
              {item.name}
            </h1>
          </Reveal>
          <Reveal delay={0.3}>
            <p className="mt-6 text-[18px] md:text-[20px] text-mist-300 leading-relaxed max-w-2xl">
              {cs.oneLiner}
            </p>
          </Reveal>
            ...
            <Reveal delay={0.45}>
              <div className="mt-8">
                <a
                  ...
                  className="inline-flex items-center gap-2 px-5 py-2.5 rounded-full bg-tide-500 hover:bg-tide-400 text-white text-[14px] font-medium transition-all shadow-glow hover:shadow-glow-lumen"
                >
      ...
      <CaseSection label="Problem">
        <p className="text-[17px] md:text-[19px] text-mist-200 leading-relaxed whitespace-pre-line">
          {cs.problem}
        </p>
      </CaseSection>
      ...
      <CaseSection label="Stack">
        <div className="flex flex-wrap gap-2">
          {cs.stack.map((s) => (
            <span
              key={s}
              className="text-[12px] md:text-[13px] px-3 py-1.5 rounded-full glass-deep text-mist-200"
            >
              {s}
            </span>
          ))}
      ...
      <section className="py-20 md:py-28">
        <div className="container-narrow">
          <div className="grid md:grid-cols-[200px_1fr] gap-10 md:gap-16 mb-10">
            ...
            <div>
              <h2 className="font-display font-medium text-[26px] md:text-[34px] leading-tight tracking-tight text-mist-100">
                The surface area we built.
              </h2>
            </div>
          </div>
          <Reveal stagger={0.07} className="grid md:grid-cols-2 gap-5">
            {cs.shipped.map((s) => (
              <div key={s.title} className="glass-deep rounded-2xl p-6 md:p-7">
                <h3 className="font-display font-medium text-[18px] text-mist-100">
                  {s.title}
                </h3>
                <p className="mt-3 text-[14px] md:text-[15px] text-mist-300 leading-relaxed">
                  {s.body}
                </p>
              </div>
            ))}
      ...
      {cs.gallery && cs.gallery.length > 0 ? (
        <section className="py-20 md:py-28">
          <div className="container-narrow">
            ...
            <Reveal stagger={0.08} className="grid md:grid-cols-2 gap-5 mt-6">
              {cs.gallery.map((g) => (
                <figure
                  key={g.src}
                ...
                >
                  ...
                  {g.caption ? (
                    <figcaption className="px-5 py-4 text-[12px] text-mist-400">
                      {g.caption}
                    </figcaption>
                  ) : null}
                </figure>
        ...
        <div className="container-narrow">
          <Reveal>
            <div className="glass-deep rounded-2xl p-8 md:p-12 text-center">
              <span className="section-label justify-center">Next</span>
              <h3 className="mt-5 font-display font-medium text-[28px] md:text-[38px] leading-tight tracking-tight text-mist-100">
              ...
              <div className="mt-8 flex flex-wrap justify-center gap-3">
                {cs.viewUrl ? (
                  <a
                    ...
                    target="_blank"
                    rel="noopener noreferrer"
                    className="inline-flex items-center gap-2 px-6 py-3 rounded-full border border-mist-300/10 text-mist-100 hover:border-lumen-400/40 hover:text-white text-[14px] font-medium transition-all"
                  >
              ...
              <p className="mt-8 text-[12px] text-mist-400">
                <Link
                  href="/#work"
...
function MetaItem({ label, value }: { label: string; value: string }) {
  return (
    <div>
      ...
      <div className="mt-2 text-[14px] md:text-[15px] text-mist-100 font-medium leading-tight">
        {value}
      </div>
    </div>
  );
