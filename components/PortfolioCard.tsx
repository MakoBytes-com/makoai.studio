...
export default function PortfolioCard({ item }: { item: PortfolioItem }) {
    ...
    <>
        ...
        {item.screenshot ? (
          ...
          <>
            ...
            <div className="absolute inset-0 flex items-center justify-center">
              <span className="font-display text-[28px] md:text-[36px] font-medium text-white/90 tracking-tight">
                {item.name}
              </span>
            </div>
          </>
      ...
      <div className="p-6">
        <div className="flex items-start justify-between gap-4">
          <div>
            <h3 className="font-display text-[21px] font-medium text-mist-100 group-hover:text-white transition-colors">
              {item.name}
            </h3>
            <p className="mt-1 text-[13px] text-lumen-300/90 font-medium">
              {item.tagline}
            </p>
          </div>
          {!archived ? (
        ...
        {item.tags.length > 0 ? (
          <div className="mt-5 flex flex-wrap gap-1.5">
            {item.tags.map((t) => (
              <span
                key={t}
                className="text-[11px] px-2.5 py-1 rounded-full bg-white/5 text-mist-300 border border-white/5"
              >
                {t}
              </span>
            ))}
        ...
        {archived && item.archivedNote ? (
          <p className="mt-5 text-[12px] text-mist-400 italic leading-relaxed pt-4">
            {item.archivedNote}
          </p>
        ) : null}
        {hasCaseStudy ? (
          <p className="mt-5 text-[12px] font-medium text-lumen-300 flex items-center gap-1.5">
            Read case study
  ...
  const shellClass = `group relative glass-deep rounded-2xl overflow-hidden block ${
    archived
      ? "cursor-default"
      : "hover:-translate-y-1 hover:shadow-card-deep"
  }`;
